...
from mmattn_triton.fused_layernorm import layernorm_adaln


# Reference PyTorch Implementation
class ReferenceLayerNormAdaLN(nn.Module):
    def __init__(self, dim, eps=1e-6):
        super().__init__()
        # In GPT2Block, if conditioning is True, elementwise_affine=False
...
def benchmark_pytorch(dim=1024, seq_len=1024, batch_size=4, num_iters=100):
    ...
    model = ReferenceLayerNormAdaLN(dim).to(device)
    
    # Verify Correctness
    print("Verifying correctness...")
    y_ref = model(x, scale, shift)
    y_triton = layernorm_adaln(x, scale, shift)
    ...
    # Benchmark Triton
    triton_ms = triton.testing.do_bench(lambda: layernorm_adaln(x, scale, shift))
